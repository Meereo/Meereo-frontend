...
router.get('/', requireAuth, async (req, res, next) => {
  try {
    ...
    const offers = await prisma.offer.findMany({
      where,
      include: {
        supplier: { select: { id: true, name: true, company: true, type: true, proProfile: { select: { entreprise: true } }, fournisseurProfile: { select: { entreprise: true } } } },
        ao: { select: { id: true, title: true, lot: true, ownerUserId: true, projectId: true } },
      },
