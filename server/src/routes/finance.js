...
router.get('/budgets', requireAuth, async (req, res, next) => {
  try {
    ...
    // FIN-01: le client doit voir les budgets liés à ses projets
...
router.get('/invoices', requireAuth, async (req, res, next) => {
  try {
    const prisma = getPrisma()
    // FIN-01: le client doit voir les factures liées à ses projets (pas seulement celles qu'il a créées)
    const userProjects = await prisma.project.findMany({
      where: { OR: [{ ownerId: req.user.id }, { clientId: req.user.id }] },
      select: { id: true },
    })
