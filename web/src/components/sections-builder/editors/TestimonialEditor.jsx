  ...
  if (sectionType === "testimonial-single") {
    return (
      <div>
        ...
        <TextField label="Nom" value={data.name} onChange={(v) => set("name", v)} />
        <TextField label="Fonction" value={data.role} onChange={(v) => set("role", v)} />
        <ImageField label="Photo" value={data.avatarSrc} onChange={(v) => set("avatarSrc", v)} />
      </div>
    );
  }
    ...
    <div>
      ...
      {data.testimonials.map((t, i) => (
        <div key={t.id} className="mb-4 p-3 bg-gray-50 rounded-xl border border-gray-100">
          <p className="text-xs font-semibold text-gray-400 mb-2">Témoignage {i + 1}</p>
          <TextField label="Citation" value={t.quote} onChange={(v) => updateTestimonial(t.id, "quote", v)} multiline />
          <TextField label="Nom" value={t.name} onChange={(v) => updateTestimonial(t.id, "name", v)} />
          <TextField label="Fonction" value={t.role} onChange={(v) => updateTestimonial(t.id, "role", v)} />
          <ImageField label="Photo" value={t.avatarSrc} onChange={(v) => updateTestimonial(t.id, "avatarSrc", v)} />
        </div>
      ))}
    </div>
  );
