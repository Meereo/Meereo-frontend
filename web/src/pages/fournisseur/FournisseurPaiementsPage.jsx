...
function PayMethodBadge({ method }) {
  ...
  const icons = { wave: '🌊', orange_money: '🟠', mtn: '🟡', mobile_money: '📱', carte: '💳', virement: '🏦', especes: '💵' }
  ...
  return (
    <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 6, background: 'var(--s2)', color: 'var(--t3)', whiteSpace: 'nowrap' }}>
      {icons[method] || '💳'} {labels[method] || method}
    </span>
  )
}
...
export default function FournisseurPaiementsPage({ showToast }) {
              ...
              <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                {detail.livMode && (
                  <span style={{ fontSize: 10, color: 'rgba(255,255,255,.4)' }}>
                    {detail.livMode === 'livraison' ? '🚚 Livraison' : detail.livMode === 'retrait' ? '🏪 Retrait' : detail.livMode}
                  </span>
                )}
              </div>
            </div>
                    ...
                    {STEP_LABELS.map((label, idx) => {
                      ...
                      return (
                        <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          {done && <span style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--ok)' }}>✓</span>}
                        </div>
                      )
                    })}
                  </div>
