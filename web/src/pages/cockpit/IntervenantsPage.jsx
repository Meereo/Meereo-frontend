...
export default function IntervenantsPage({ showToast, openModal }) {
      ...
      <div className="three-col">
        {filtered.length === 0 && (
          <div style={{ gridColumn: '1 / -1' }}>
            <DSEmptyState icon={<HardHat size={24}/>} title="Aucun intervenant référencé" description="Les prestataires assignés à vos projets apparaîtront ici automatiquement." actionLabel="+ Intervenant" onAction={() => openModal('newInter')} />
          </div>
        )}
        {filtered.map(m => {
          ...
          return (
            <div key={m.id} className="card" style={{ padding: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 12 }}>
                ...
                <button className="btn btn-sm" style={{ fontSize: 11, padding: '5px 10px' }} onClick={() => { openModal && openModal('newMessage'); showToast && showToast('Nouveau message pour ' + m.nom) }}>Contacter</button>
