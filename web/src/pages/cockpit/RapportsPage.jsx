...
export default function RapportsPage({ openModal, showToast }) {
    ...
    <div>
      ...
      <div className="split">
        ...
        <div className="split-left">
          ...
          {filtered.length === 0 && (
            <DSEmptyState icon={<ClipboardList size={24}/>} title="Aucun rapport" description="Créez votre premier rapport de chantier." actionLabel="+ Nouveau rapport" onAction={() => openModal('newRapport')} />
          )}
          {filtered.map(r => (
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  ...
                  <button className="btn btn-sm" onClick={() => showToast && showToast('Telechargement PDF...')}>↓ PDF</button>
                </div>
              </div>
                  ...
                  <div className="rg-3" style={{ gap: 8 }}>
                    {selected.medias.map((m, i) => (
                      <div key={i} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', aspectRatio: '4/3' }}>
                        ...
                        {m.type === 'video' && <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 32, height: 32, borderRadius: '50%', background: 'rgba(0,0,0,.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 14 }}>▶</div>}
                      </div>
                    ))}
                  </div>
                ) : (
