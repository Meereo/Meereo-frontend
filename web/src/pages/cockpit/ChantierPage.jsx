...
export default function ChantierPage({ openModal, showToast, onNavigate }) {
  ...
  return (
    <div>
      <DSPageHeader title="Suivi chantier" subtitle={proj ? `${proj.nom} · ${PHASE_LABELS[normalizePhase(proj.phase)] || proj.phase || 'Esquisse'} · ${globalPct}%` : 'Sélectionnez un projet'}>
        <button className="btn btn-sm" onClick={() => openModal && openModal('newRapport')}>Rapport</button>
        <button className="btn btn-primary btn-sm" onClick={() => openModal && openModal('newNote')}>+ Note de chantier</button>
      </DSPageHeader>
                    ...
                    {isOpen && (
                      <div style={{ borderTop: '1px solid var(--border)' }}>
                        {ph.tasks.map((t, ti) => {
                          ...
                          return (
                            <div key={ti} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', borderBottom: ti < ph.tasks.length - 1 ? '1px solid var(--border)' : 'none', borderLeft: st === 'active' ? '3px solid var(--tx)' : '3px solid transparent', background: st === 'active' ? 'rgba(0,0,0,.02)' : undefined }}>
                              ...
                              <button onClick={() => openModal && openModal('newNote')} style={{ width: 26, height: 26, borderRadius: 6, background: 'var(--s2)', border: '1px solid var(--border-card)', color: 'var(--t3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }} title="Ajouter une note">
                        ...
                        <div style={{ display: 'flex', gap: 8, padding: '10px 16px', background: 'var(--s2)', borderTop: '1px solid var(--border)' }}>
                          ...
                          <button className="btn btn-sm" style={{ fontSize: 10, padding: '4px 10px' }} onClick={() => openModal && openModal('newNote')}>+ Note</button>
                        </div>
                      </div>
                    )}
                  </div>
