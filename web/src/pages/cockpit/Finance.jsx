...
import { PROJECT_PHASES, PHASE_LABELS } from '../../domain/status'
...
// FIN-01: circuit de facture — Émission (pro) → Réception (client) → Déclaration paiement (client) → Confirmation (pro = réglée)
const INVOICE_TRANSITIONS = {
  brouillon:         { next: 'emise',             label: 'Émettre',                   actor: 'pro' },
  emise:             { next: 'recue',             label: 'Marquer comme reçue',        actor: 'client' },
  recue:             { next: 'paiement_declare',  label: 'Déclarer le paiement',       actor: 'client' },
  paiement_declare:  { next: 'reglee',            label: 'Confirmer la réception',     actor: 'pro' },
}

const INVOICE_BADGE = {
  brouillon:        { label: 'Brouillon',           color: 'var(--t4)',  bg: 'var(--s2)' },
  emise:            { label: 'Émise',                color: '#007AFF',    bg: 'rgba(0,122,255,.07)' },
  recue:            { label: 'Reçue par le client',  color: '#E07B00',    bg: 'rgba(245,158,11,.07)' },
...
const EXPENSE_CATEGORIES = ['Main d\'oeuvre','Matériaux','Transport','études','Honoraires','équipements','Sous-traitance','Divers']
...
export default function Finance({ showToast }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      // FIN-04: fusionner budgets Finance + budgets projet
      const financeBudgets = b || []
      const projectsWithBudget = (store.projects || []).filter(p => {
        const bv = parseFloat(String(p.budget || '0').replace(/\s/g, ''))
        return bv > 0 && !financeBudgets.some(fb => fb.projectId === p.id)
      })
      const virtualBudgets = projectsWithBudget.map(p => ({
        id: 'virt_' + p.id, label: 'Budget ' + (p.nom || 'projet'),
        montant: parseFloat(String(p.budget || '0').replace(/\s/g, '')),
        projectId: p.id, ownerId: p.ownerId, createdAt: p.createdAt, _fromProject: true,
      }))
      setBudgets([...financeBudgets, ...virtualBudgets])
      setExpenses(e || [])
      setInvoices(i || [])
    } catch (err) {
  ...
  const handleSaveInvoice = async () => {
    setFormSubmitted(true)
    if (!form.label || !form.montant || !form.phase || !form.dateFacture || !form.modePaiement) return
    try {
      if (modal.mode === 'create') {
        await api.finance.createInvoice({ label: form.label, montant: form.montant, provider: form.provider || null, projectId: form.projectId || null, phase: form.phase, dateFacture: form.dateFacture, modePaiement: form.modePaiement })
        showToast && showToast('Facture créée')
      } else {
        await api.finance.updateInvoice(modal.item.id, { label: form.label, montant: form.montant, provider: form.provider, phase: form.phase, dateFacture: form.dateFacture, modePaiement: form.modePaiement })
    ...
    <div>
      ...
      {tab === 'budgets' && (
        loading ? (
          ...
          <DSEmptyState icon="$" title="Aucun budget" description="Créez votre premier budget pour suivre les dépenses par projet." actionLabel="Créer un budget" onAction={() => openCreate('budget')} />
        ) : (
        ...
        budgets.length === 0 ? (
          ...
          <div>
            ...
            {loading ? (
              ...
              <DSEmptyState icon="*" title="Aucune dépense" description="Enregistrez vos premières dépenses de projet." actionLabel="Ajouter une dépense" onAction={() => openCreate('expense')} />
            ) : (
              <div className="card" style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', minWidth:560, borderCollapse:'collapse', fontSize:13 }}>
                  <thead>
      ...
      {tab === 'factures' && (
        <div>
          ...
          {loading ? (
            ...
            <DSEmptyState icon="$" title="Aucune facture" description="Créez votre première facture." actionLabel="Nouvelle facture" onAction={() => openCreate('invoice')} />
          ) : (
            <div className="card" style={{ overflowX:'auto' }}>
      ...
      {tab === 'rapports' && (
        <div>
          ...
          {!selectedProjectId ? (
            <DSEmptyState icon="*" title="Sélectionnez un projet" description="Choisissez un projet pour afficher les indicateurs financiers détaillés." />
          ) : reportLoading ? (
            <Spinner />
          ) : !report ? (
            <DSEmptyState icon="*" title="Aucune donnée" description="Aucune transaction enregistrée pour ce projet." />
          ) : (
            <div>
              {/* KPIs */}
      ...
      <Modal isOpen={modal?.type === 'budget'} onClose={closeModal} title={modal?.mode === 'edit' ? 'Modifier le budget' : 'Nouveau budget'}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div>
            ...
            <input className="form-input" value={form.label || ''} onChange={e => pf('label', e.target.value)} placeholder="ex: Budget gros oeuvre" />
          </div>
          <div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <div className="modal-row" style={{ gap:10 }}>
            ...
            <div>
              ...
              <select className="form-input" value={form.phase || ''} onChange={e => pf('phase', e.target.value)}>
                <option value="">Choisir une phase</option>
                {PROJECT_PHASES.map(ph => <option key={ph} value={ph}>{PHASE_LABELS[ph] || ph}</option>)}
              </select>
              {formSubmitted && !form.phase && <p style={{ color:'var(--err)', fontSize:11, marginTop:4, fontWeight:500 }}>Phase obligatoire</p>}
            </div>
          </div>
          <div className="modal-row" style={{ gap:10 }}>
            <div>
              <label className="form-label">Date de facture *</label>
              <input className="form-input" type="date" value={form.dateFacture || ''} onChange={e => pf('dateFacture', e.target.value)} />
              {formSubmitted && !form.dateFacture && <p style={{ color:'var(--err)', fontSize:11, marginTop:4, fontWeight:500 }}>Date obligatoire</p>}
            </div>
            <div>
              <label className="form-label">Mode de paiement *</label>
              <select className="form-input" value={form.modePaiement || ''} onChange={e => pf('modePaiement', e.target.value)}>
              ...
              {formSubmitted && !form.modePaiement && <p style={{ color:'var(--err)', fontSize:11, marginTop:4, fontWeight:500 }}>Mode de paiement obligatoire</p>}
            </div>
          </div>
        </div>
      </Modal>
