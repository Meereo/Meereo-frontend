...
const PHASE_ICON_MAP = {
  '�Ÿ“�': <Ruler size={14}/>,
  '�Ÿ“‹': <ClipboardList size={14}/>,
  '�Ÿ�—️': <HardHat size={14}/>,
  '�Ÿ”�': <Wrench size={14}/>,
  '�Ÿ“�': <Package size={14}/>,
  '�Ÿ›‹️': <Sofa size={14}/>,
  '�œ…': <CheckCircle2 size={14}/>,
}
...
function ReportModal({ isOpen, onClose, showToast }) {
  ...
  const submit = () => {
    ...
    emitEvent('document_uploaded', { name: f.type }, { notifMsg: `Rapport cr�� : ${f.type}` })
    showToast('Rapport cr��')
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Nouveau rapport" footer={<><button className="btn btn-sm" onClick={onClose}>Annuler</button><button className="btn btn-primary btn-sm" onClick={submit}>Enregistrer</button></>}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div className="form-row">
          <div><label className="form-label">Type de rapport</label><select className="form-input" value={f.type} onChange={e => setF(p => ({ ...p, type: e.target.value }))}><option>Rapport hebdomadaire</option><option>Rapport de visite</option><option>Compte-rendu r�union</option><option>Rapport technique</option><option>Rapport mensuel</option><option>Rapport de chantier</option><option>PV de r�ception</option></select></div>
          <div><label className="form-label">Projet</label><select className="form-input" value={f.projet} onChange={e => setF(p => ({ ...p, projet: e.target.value }))}><option value="">S�lectionner</option>{(store.projects || []).map(p => <option key={p.id} value={p.id}>{p.nom}</option>)}</select></div>
        </div>
        <div className="form-row">
        ...
        <div><label className="form-label">Participants</label><input className="form-input" placeholder="Noms s�par�s par des virgules" value={f.participants} onChange={e => setF(p => ({ ...p, participants: e.target.value }))} /></div>
...
2. Points techniques" value={f.ordre} onChange={e => setF(p => ({ ...p, ordre: e.target.value }))} /></div>
        <div><label className="form-label">D�cisions prises</label><textarea className="form-input" rows="2" placeholder="D�cisions et actions..." value={f.decisions} onChange={e => setF(p => ({ ...p, decisions: e.target.value }))} /></div>
        ...
        <div><label className="form-label">Prochaine r�union</label><input className="form-input" type="date" value={f.prochaine} onChange={e => setF(p => ({ ...p, prochaine: e.target.value }))} /></div>
      </div>
    </Modal>
  )
}
...
function NoteModal({ isOpen, onClose, showToast }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Note de chantier" footer={<><button className="btn btn-sm" onClick={onClose}>Annuler</button><button className="btn btn-primary btn-sm" onClick={submit}>Enregistrer</button></>}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div className="form-row">
          <div><label className="form-label">T�che concern�e</label><input className="form-input" placeholder="Nom de la t�che..." value={f.tache} onChange={e => setF(p => ({ ...p, tache: e.target.value }))} /></div>
          <div><label className="form-label">Statut</label><select className="form-input" value={f.statut} onChange={e => setF(p => ({ ...p, statut: e.target.value }))}><option>Termin�</option><option>En cours</option><option>� faire</option><option>Bloqu�</option></select></div>
        </div>
        ...
        <div><label className="form-label">Note / Rapport</label><textarea className="form-input" style={{ resize: 'vertical', minHeight: 80 }} rows="4" placeholder="Avancement constat�, points de vigilance..." value={f.texte} onChange={e => setF(p => ({ ...p, texte: e.target.value }))} /></div>
      </div>
    </Modal>
  )
}
...
export default function Worksite({ openModal, showToast, onNavigate }) {
  ...
  // �•��•��•� T�‚CHES R�ELLES DU PROJET (backend) �•��•��•�
  const [projTasks, setProjTasks] = useState([])

  // Recharger les t�ches d�s que le projet change
  useEffect(() => {
    if (!selProjId) { setProjTasks([]); return }
    api.tasks.getByProject(selProjId).then(t => setProjTasks(Array.isArray(t) ? t : [])).catch(() => {})
  }, [selProjId])

  // Fusionner t�ches backend + t�ches locales (cr��es optimistiquement via acceptOffer)
  ...
  // Mise � jour statut d'une t�che r�elle — optimiste + sync API
  ...
  const cycleTask = (taskId) => {
    ...
    setTaskStates(prev => {
      ...
      if (proj && proj.id) {
        ...
        // Task-based avancement — source de v�rit� unique
        ...
        // �tapes sync (for phase tracking, if �tapes exist)
        ...
        // Store the MAX of task-based and �tapes-based avancement
  ...
  // KPI counts — prioritise les t�ches r�elles backend si disponibles
  ...
  const doAssign = (nom, id) => {
    ...
    if (assignModal.taskId) {
      ...
      if (ph) {
        ...
        showToast && showToast(nom + ' assign� � toute la phase ' + ph.name)
      }
    }
  ...
  const doCreateInter = () => {
    ...
    showToast && showToast(newInter.nom + ' cr�� et assign�')
    ...
    return (
      <div>
        ...
        <div style={{ textAlign: 'center', padding: '48px 24px' }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--tx)', marginBottom: 4 }}>Aucun projet � suivre</div>
          <div style={{ fontSize: 12, color: 'var(--t3)', marginBottom: 16 }}>Cr�ez un projet pour d�marrer le suivi de chantier.</div>
  ...
  return (
    <div>
      <DSPageHeader title="Suivi chantier" subtitle={proj ? `${proj.nom} � ${PHASE_LABELS[normalizePhase(proj.phase)] || proj.phase || 'Esquisse'} � ${globalPct}%` : 'S�lectionnez un projet'}>
          ...
          {!proj ? (
            ...
            <div>
              ...
              <div className="rg-4" style={{ gap: 10, marginBottom: 16 }}>
                {[
                  { v: doneTasks + '/' + totalTasks, l: 'Termin�es', svg: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg>, iconBg: 'rgba(52,199,89,.08)', iconColor: 'var(--ok)' },
                  ...
                  { v: blockedTasks, l: 'Bloqu�es', svg: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>, iconBg: 'rgba(255,59,48,.07)', iconColor: 'var(--err)' },
                  { v: Object.keys(assignments).length, l: 'Assign�s', svg: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>, iconBg: 'rgba(124,58,237,.07)', iconColor: '#7C3AED' },
                ].map((k, i) => (
              ...
              {(() => {
                ...
                if (cs === 'CLOTURE_VALIDE_EXTERNE' || cs === 'CLOTURE_VALIDE_MEEREO') return (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', marginBottom: 16, background: 'rgba(52,199,89,.05)', border: '1px solid rgba(52,199,89,.15)', borderRadius: 12 }}>
                    ...
                    <div><div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ok)' }}>Cl�tur� — valid� par le client</div></div>
                  </div>
                )
                if (cs === 'DEMANDE_CLOTURE_ENVOYEE' || cs === 'EN_ATTENTE_VALIDATION_CLIENT') return (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', marginBottom: 16, background: 'rgba(255,149,0,.04)', border: '1px solid rgba(255,149,0,.12)', borderRadius: 12 }}>
                    ...
                    <div><div style={{ fontSize: 13, fontWeight: 700, color: '#FF9500' }}>Demande envoy�e — en attente de validation client</div></div>
                  </div>
                )
                if (cs === 'CLOTURE_REFUSEE') return (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', marginBottom: 16, background: 'rgba(255,59,48,.04)', border: '1px solid rgba(255,59,48,.12)', borderRadius: 12 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--err)' }}>Cl�ture refus�e par le client</div>
                ...
                return (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', marginBottom: 16, background: 'var(--surface-1)', border: '1px solid var(--border-card)', borderRadius: 12 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 700 }}>Cl�ture du projet</div>
              ...
              {/* T�‚CHES CONTRACTUELLES (depuis le backend) */}
              {tasksByMarket.length > 0 && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--t4)', marginBottom: 10 }}>T�ches contractuelles</div>
                        ...
                        {mkt && (
                          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', background: 'var(--s2)', display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: 12.5, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{mkt.lot || mkt.titre || 'March�'}</div>
                        ...
                        {group.tasks.map((t, ti) => {
                          ...
                          return (
                            <div key={t.id || ti} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', borderBottom: ti < group.tasks.length - 1 ? '1px solid var(--border)' : 'none', borderLeft: isActive ? '3px solid var(--tx)' : '3px solid transparent', background: isActive ? 'rgba(0,0,0,.02)' : undefined }}>
                              ...
                              <span style={{ fontSize: 9.5, fontWeight: 700, padding: '2px 7px', borderRadius: 100, background: stBg, color: stColor, flexShrink: 0 }}>{isDone ? 'Termin�' : isActive ? 'En cours' : '� faire'}</span>
                            </div>
                          )
                        })}
                      </div>
                    ...
                    return (
                      <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, zIndex: 1, flex: 1, cursor: 'pointer' }} onClick={() => togglePhase(i)}>
                        ...
                        <div style={{ fontSize: 8, fontWeight: 700, color: phAllDone ? 'var(--ok)' : phStarted ? (ph.color || 'var(--tx)') : 'var(--t4)' }}>
                          {phAllDone ? <><Check size={8}/> Termin�</> : phStarted ? Math.round(ph.tasks.filter(t => getTaskState(t.id) === 'done').length / ph.tasks.length * 100) + '%' : 'A venir'}
                        </div>
                      </div>
                    )
                  })}
                  ...
                  <div key={phIdx} className="card" style={{ marginBottom: 10, overflow: 'hidden' }}>
                    ...
                    <div onClick={() => togglePhase(phIdx)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px', cursor: 'pointer' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                        {isDone && <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 100, background: 'var(--s2)', color: 'var(--tx)', display: 'inline-flex', alignItems: 'center', gap: 3 }}><Check size={10}/> Termin�</span>}
                        {isAct && !isDone && <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 100, background: 'rgba(0,0,0,.06)', color: 'var(--tx)' }}>�—� En cours</span>}
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                {partner && (
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 3 }}>
                                    ...
                                    {evalDone.includes(t.id) && <span style={{ fontSize: 9, fontWeight: 600, padding: '2px 6px', borderRadius: 100, background: 'rgba(52,199,89,.08)', color: 'var(--ok)', display: 'inline-flex', alignItems: 'center', gap: 3 }}><Check size={9}/> Evalu�</span>}
                                  </div>
                                )}
                              </div>
                              {/* Status badge */}
          ...
          <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 16, width: 580, maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,.18)' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '20px 22px 14px', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                {[['plateforme', 'Rechercher sur la plateforme'], ['inviter', 'Inviter par email'], ['creer', 'Cr�er un profil']].map(([k, l]) => (
            ...
            <div style={{ flex: 1, overflowY: 'auto', borderTop: '1px solid var(--border)' }}>
              ...
              {assignTab === 'plateforme' && (
                <div>
                    ...
                    return (
                      <div key={p.id + p.source} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 22px', borderBottom: '1px solid var(--border)', cursor: 'pointer', transition: 'background .1s' }} onClick={() => doAssign(p.nom, p.id)}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 1 }}>{p.specialite}{p.ville ? ' � ' + p.ville : ''}</div>
                        </div>
                  ...
                  {filteredPartners.length === 0 && <div style={{ padding: '24px 22px', textAlign: 'center', fontSize: 12, color: 'var(--t4)' }}>Aucun r�sultat — essayez l'onglet "Inviter par email"</div>}
                </div>
              )}
              ...
              {assignTab === 'inviter' && (
                <div style={{ padding: '22px' }}>
                  <div style={{ background: 'var(--s2)', borderRadius: 12, padding: 20, marginBottom: 16 }}>
                    ...
                    <div style={{ fontSize: 11.5, color: 'var(--t3)', lineHeight: 1.5 }}>L'intervenant recevra une invitation par email pour rejoindre la plateforme MEEREO et sera automatiquement assign� � cette t�che.</div>
                  </div>
                  ...
                  <div style={{ marginTop: 20, padding: '14px 16px', background: 'rgba(124,58,237,.04)', borderRadius: 10, border: '1px solid rgba(124,58,237,.1)' }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--t3)', lineHeight: 1.5 }}>Vous pouvez cr�er un profil manuellement dans l'onglet "Cr�er un profil", avec ou sans email.</div>
                  </div>
                </div>
              )}
              ...
              {assignTab === 'creer' && (
                <div style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ background: 'var(--s2)', borderRadius: 12, padding: 16, marginBottom: 4 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 2 }}>Cr�er un profil intervenant</div>
                    <div style={{ fontSize: 11, color: 'var(--t3)' }}>Ce profil sera ajout� � votre base d'intervenants et pourra être r�utilis� sur d'autres projets.</div>
                  </div>
                  ...
                  <div><label className="form-label">R�le / Sp�cialit�</label><input className="form-input" value={newInter.role} onChange={e => setNewInter(p => ({ ...p, role: e.target.value }))} placeholder="Electricien, Plombier, Architecte..." /></div>
                  ...
                  <button className="btn btn-primary btn-sm" style={{ alignSelf: 'flex-end', marginTop: 4 }} onClick={doCreateInter} disabled={!newInter.nom.trim()}>Cr�er et assigner</button>
                </div>
              )}
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,.4)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'modalIn .18s ease' }} onClick={() => setEvalPresta(null)}>
          <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 16, width: 460, boxShadow: '0 24px 80px rgba(0,0,0,.18)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '22px 24px 16px', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--t3)' }}>{evalPresta.nom} � Mission terminee</div>
            </div>
            ...
            <div style={{ padding: '14px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button style={{ padding: '8px 18px', borderRadius: 10, background: 'var(--tx)', color: '#fff', border: 'none', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--f)', fontSize: 13 }} onClick={() => {
                ...
                setEvalPresta(null)
                showToast && showToast('�valuation enregistr�e � Note mise � jour pour ' + evalPresta.nom)
      ...
      {/* �•��•��•� MODAL: Demande de cl�ture (3 options) �•��•��•� */}
      {confirmModal && (() => {
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,.4)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'modalIn .18s ease' }} onClick={() => setConfirmModal(null)}>
          <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 16, width: 500, maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(0,0,0,.18)' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '22px 24px 16px' }}>
              <div style={{ fontSize: 17, fontWeight: 800, letterSpacing: '-.3px', marginBottom: 4 }}>Cl�ture du projet</div>
            ...
            <div style={{ padding: '0 24px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {/* Option 1 — Lien de validation (recommand�e) */}
              <div style={{ padding: '16px 18px', border: '2px solid var(--tx)', borderRadius: 12, cursor: 'pointer', background: 'rgba(0,0,0,.01)' }} onClick={() => {
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  ...
                  <span style={{ marginLeft: 'auto', fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 100, background: 'var(--tx)', color: '#fff' }}>RECOMMAND�</span>
                </div>
                <div style={{ fontSize: 11.5, color: 'var(--t3)', lineHeight: 1.5 }}>Le client recevra un lien simple pour confirmer la r�ception du projet.</div>
              </div>
              ...
              <div style={{ padding: '16px 18px', border: '1px solid var(--border-card)', borderRadius: 12, cursor: 'pointer' }} onClick={() => {
                const email = prompt('Email du client � inviter sur MEEREO :')
                if (!email || !email.includes('@')) return
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700 }}>Inviter le client � rejoindre MEEREO</div>
                </div>
              ...
              <div style={{ padding: '16px 18px', border: '1px solid var(--border-card)', borderRadius: 12, cursor: 'pointer' }} onClick={() => {
                ...
                <div style={{ fontSize: 11.5, color: 'var(--t3)', lineHeight: 1.5 }}>Vous avez d�j� obtenu la validation du client hors plateforme. Joignez une preuve (photo, PDF, message).</div>
              </div>
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,.45)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'modalIn .18s ease' }} onClick={() => setClientRatingModal(null)}>
          <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 16, width: 500, maxHeight: '85vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,.2)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '22px 24px 16px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--t3)', marginTop: 3 }}>{clientRatingModal.projNom} � {clientRatingModal.client}</div>
                </div>
            ...
            <div style={{ padding: '14px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8, justifyContent: 'flex-end', flexShrink: 0 }}>
              ...
              <button className="btn btn-primary btn-sm" onClick={() => { setClientRatingModal(null); showToast && showToast('Merci ! �valuation enregistr�e � Le ranking du prestataire a �t� mis � jour') }}>Envoyer mon �valuation</button>
            </div>
          </div>
        </div>
      )}
