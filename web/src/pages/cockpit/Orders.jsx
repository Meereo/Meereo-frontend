...
export default function Orders({ onNavigate, showToast, openModal }) {
  ...
  return (
    <div>
      <DSPageHeader title="Commandes" subtitle={`${total} commandes · ${enCours} en cours`}>
        ...
        <button className="btn btn-primary btn-sm" onClick={() => setShowCreateOrder(true)}>+ Nouvelle commande</button>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 20px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                    <span style={{ fontFamily: 'monospace', fontSize: 11, fontWeight: 600, color: 'var(--tx)' }}>{c.ref}</span>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0, marginRight: 8 }}>
                  <div style={{ fontSize: 15, fontWeight: 600, letterSpacing: '-.4px' }}>{fmtMoney(c.montant)}</div>
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0, alignItems: 'center' }}>
                  ...
                  {isActive && c.livMode === 'livraison' && <button style={{ fontSize: 10, padding: '5px 10px', borderRadius: 6, background: 'var(--tx)', color: '#fff', border: 'none', cursor: 'pointer', fontFamily: 'var(--f)', fontWeight: 600 }} onClick={() => setTracking(c)}>Suivre</button>}
                </div>
              </div>
            </div>
          )
      ...
      {/* â•â•â• Modal: Detail commande â•â•â• */}
      {detail && (
          ...
          <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 16, width: 480, boxShadow: '0 24px 80px rgba(0,0,0,.18)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '20px 24px', background: 'linear-gradient(145deg,#0f1011,#2a2c2d)', color: '#fff' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 600 }}>{detail.designation}</div>
                  <div style={{ fontSize: 12, color: 'rgba(255,255,255,.5)', marginTop: 3 }}>{detail.fournisseur} · {formatDateFR(detail.date)}</div>
                </div>
                <button onClick={() => setDetail(null)} style={{ width: 30, height: 30, borderRadius: 8, background: 'rgba(255,255,255,.1)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: 'rgba(255,255,255,.6)' }}>À</button>
              </div>
              <div style={{ fontSize: 24, fontWeight: 600, marginTop: 12 }}>{fmtMoney(detail.montant)}</div>
            </div>
            <div style={{ padding: '18px 24px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div className="modal-row" style={{ gap: 8 }}>
                <div style={{ padding: '10px 12px', background: 'var(--s2)', borderRadius: 8 }}>
                  <div style={{ fontSize: 9, fontWeight: 600, color: 'var(--t4)', textTransform: 'uppercase', marginBottom: 3 }}>Projet</div>
                  <div style={{ fontSize: 12, fontWeight: 600 }}>{detail.projet}</div>
                </div>
                <div style={{ padding: '10px 12px', background: 'var(--s2)', borderRadius: 8 }}>
                  <div style={{ fontSize: 9, fontWeight: 600, color: 'var(--t4)', textTransform: 'uppercase', marginBottom: 3 }}>Livraison</div>
                  <div style={{ fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>{detail.livMode === 'livraison' ? <><Truck size={12}/> Livraison MEEREO</> : <><Store size={12}/> Retrait</>}</div>
                </div>
              </div>
              {detail.partner && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: 'var(--s2)', borderRadius: 8 }}>
                  <Truck size={18}/>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, fontWeight: 600 }}>{detail.partner.nom}</div>
            ...
            <div style={{ padding: '14px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8 }}>
              {detail.step < 5 && detail.livMode === 'livraison' && (
                <button style={{ flex: 1, padding: '10px 16px', borderRadius: 10, background: 'var(--tx)', color: '#fff', border: 'none', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--f)', fontSize: 13 }} onClick={() => { setDetail(null); setTracking(detail) }}>Suivre en direct</button>
              )}
              {detail.step < 5 && (
                <button style={{ flex: 1, padding: '10px 16px', borderRadius: 10, background: 'var(--ok)', color: '#fff', border: 'none', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--f)', fontSize: 13 }} onClick={() => { const cmd = { ...detail, step: 5, statut: 'livre' }; setDetail(null); setEvalFourn({ cmd, ratings: { stars: 0, qualite: 0, delai: 0, emballage: 0, comment: '' } }) }}>Confirmer reception</button>
              )}
              {detail.step === 5 && (
      ...
      {/* â•â•â• Modal: Tracking GPS â•â•â• */}
      {tracking && (
          ...
          <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 16, width: 560, maxHeight: '85vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 80px rgba(0,0,0,.25)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '20px 24px', background: 'linear-gradient(145deg,#0f1011,#2a2c2d)', color: '#fff', flexShrink: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                <div>
                  ...
                  <div style={{ fontSize: 17, fontWeight: 600 }}>{tracking.ref}</div>
                ...
                <button onClick={() => setTracking(null)} style={{ width: 30, height: 30, borderRadius: 8, background: 'rgba(255,255,255,.1)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: 'rgba(255,255,255,.6)' }}>À</button>
              </div>
              {/* GPS Map */}
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
                ...
                return (
                  <div key={s.id} style={{ display: 'flex', gap: 14 }}>
                    ...
                    <div style={{ paddingBottom: 12 }}>
                      ...
                      {current && <div style={{ fontSize: 10, color: '#F59E0B', fontWeight: 600, marginTop: 2 }}>â— En cours...</div>}
                    </div>
                  </div>
                )
              })}
              ...
              {tracking.partner && (
                <div style={{ padding: '12px 14px', background: 'var(--s2)', borderRadius: 10, marginTop: 8 }}>
                  <div style={{ fontSize: 9, fontWeight: 600, color: 'var(--t4)', textTransform: 'uppercase', marginBottom: 6 }}>Partenaire logistique</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 12, fontWeight: 600 }}>{tracking.partner.nom}</div>
            ...
            {tracking.step < 5 && (
              <div style={{ padding: '14px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 8, flexShrink: 0 }}>
                ...
                <button style={{ flex: 1, padding: '10px 16px', borderRadius: 10, background: 'var(--tx)', color: '#fff', border: 'none', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--f)', fontSize: 13 }} onClick={() => { const cmd = { ...tracking, step: 5, statut: 'livre' }; setTracking(null); setEvalFourn({ cmd, ratings: { stars: 0, qualite: 0, delai: 0, emballage: 0, comment: '' } }) }}>Confirmer reception</button>
              </div>
            )}
          </div>
        </div>
      ...
      {/* â•â•â• Modal: Evaluer le fournisseur â•â•â• */}
      {evalFourn && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 2200, background: 'rgba(0,0,0,.45)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'modalIn .18s ease' }} onClick={() => { setEvalFourn(null); showToast && showToast('Réception confirmée') }}>
          <div style={{ background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: 16, width: 460, boxShadow: '0 24px 80px rgba(0,0,0,.2)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '22px 24px 16px', borderBottom: '1px solid var(--border)' }}>
              <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 4 }}>Evaluer le fournisseur</div>
            ...
            <div style={{ padding: '20px 24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: 'var(--s2)', borderRadius: 10, marginBottom: 16 }}>
                {evalFourn.cmd.img ? (
                  ...
                  <div style={{ width: 40, height: 40, borderRadius: 8, background: 'var(--tx)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 600, color: '#fff', flexShrink: 0 }}>{evalFourn.cmd.fournisseur.split(' ').slice(0, 2).map(w => w[0]).join('').toUpperCase()}</div>
                )}
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{evalFourn.cmd.designation}</div>
      ...
      <OrderModal isOpen={showCreateOrder} onClose={() => setShowCreateOrder(false)} showToast={showToast} />
    </div>
  )
}
