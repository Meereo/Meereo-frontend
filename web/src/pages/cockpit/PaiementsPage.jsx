...
export default function PaiementsPage({ showToast }) {
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <DSEmptyState icon="💳" title="Aucun paiement" description="Créez un ordre de paiement pour sécuriser un règlement." actionLabel="Nouveau paiement" onAction={() => setShowCreate(true)} />
      ) : (
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'20px 24px', display:'flex', flexDirection:'column', gap:20 }}>
                ...
                {selected && (() => {
                  const transitions = {
                    ...
                    DISPUTE_OPEN:       [{ to:'PAYOUT_DONE',      label:'Résoudre → verser' },  { to:'REVERSED',    label:'Résoudre → rembourser' }],
                  }
                          ...
                          <div key={m.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', background:'var(--s2)', borderRadius:10 }}>
                            ...
                            {m.status !== 'completed' && (
                              <button className="btn btn-sm" style={{ fontSize:9, padding:'3px 8px' }} onClick={() => handleMilestoneUpdate(m.id, { status: m.status === 'pending' ? 'in_progress' : 'completed' })}>
                                {m.status === 'pending' ? '▶ Démarrer' : '✓ Terminer'}
                              </button>
                            )}
                          </div>
                        )
          ...
          <div style={{ borderTop:'1px solid var(--border)', paddingTop:14 }}>
            ...
            {form.milestones.map((ml, i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 10px', background:'var(--s2)', borderRadius:8, marginBottom:6 }}>
                ...
                <button onClick={() => setForm(p => ({ ...p, milestones:p.milestones.filter((_, j) => j !== i) }))} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--err)', fontSize:14 }}>×</button>
              </div>
            ))}
