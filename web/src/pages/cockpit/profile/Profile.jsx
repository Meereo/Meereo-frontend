...
const DEFAULT_SERVICES = [
  'Maîtrise d\'Å“uvre', 'Plans & permis', 'Études structure', 'Suivi de chantier',
...
export default function Profile() {
  ...
  // â”€â”€ Données publiques (chargées depuis le backend via UUID) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Détection rôle : propriétaire = pro connecté dont publicId = uuid â”€â”€
  ...
  // â”€â”€ Source de données : backend si uuid fourni, sinon store local â”€â”€â”€â”€â”€â”€â”€â”€
  // Pour le propriétaire sans uuid â†’ utilise store. Sinon â†’ pubData.profile
  ...
  // MÃªme source que le cockpit Parametres > Equipe
  ...
  // Si cockpitTeam existe dans le store â†’ l'utiliser. Sinon fallback = équipe cockpit par défaut (PAS la démo TEAM)
    ...
    <div className="pp-page">
      ...
      {pubError && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', flexDirection: 'column', gap: 12 }}>
          <div style={{ fontSize: 32, opacity: .3 }}>ðŸ”</div>
      ...
      <div className="pp-body">
        ...
        {(() => {
          ...
          // Si profil public chargé depuis backend â†’ utilise les stats agregees
            ...
            {proBio ? (
              ...
              <div style={{ padding: '16px 0', color: 'var(--t4)', fontSize: 13 }}>
                ...
                {isOwner && <button className="pp-see-all" style={{ marginTop: 8 }} onClick={() => {
                  ...
                  setEditSecteurs(ob.secteurs || []); setEditLogoColor(ob.logoColor || '#1D1D1F')
                  setEditModal('profil')
                }}>Ajouter une présentation â†’</button>}
              </div>
            )}
            ...
            <div className="pp-services-grid" style={{ marginTop: 12 }}>
              {proServices.length > 0 ? proServices.map(s => <span key={s} className="pp-service-tag">{s}</span>) : (
                <div style={{ padding: '8px 0', color: 'var(--t4)', fontSize: 12 }}>
                  ...
                  {isOwner && <button className="pp-see-all" style={{ marginLeft: 8 }} onClick={() => { setEditServices([]); setEditModal('services') }}>Ajouter â†’</button>}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="pp-section" id="pp-portfolio">
          <div className="pp-section-hdr">
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {isOwner && <button className="pp-see-all" onClick={() => { setEditPortfolio(displayPortfolio.map(p => ({ ...p, imgs: p.imgs?.length > 0 ? p.imgs : (p.img ? [p.img] : []) }))); setEditModal('portfolio') }}>Gerer â†’</button>}
            </div>
          </div>
          {displayPortfolio.length > 0 ? (
            ...
            <div style={{ textAlign: 'center', padding: '32px 24px', background: 'var(--s2)', borderRadius: 12 }}>
              ...
              {isOwner && <button className="pp-see-all" onClick={() => { setEditPortfolio([]); setEditModal('portfolio') }}>Ajouter des realisations â†’</button>}
            </div>
          )}
        </div>
        ...
        <div className="pp-section">
          <div className="pp-section-hdr">
            ...
            {isOwner && <button className="pp-see-all" onClick={() => { setEditTeamList(rawTeam.map(t => ({ ...t }))); setEditModal('equipe') }}>Gerer â†’</button>}
          </div>
          {displayTeam.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '32px 24px', background: 'var(--s2)', borderRadius: 12 }}>
              ...
              {isOwner && <button className="pp-see-all" onClick={() => { setEditTeamList([]); setEditModal('equipe') }}>Ajouter des membres â†’</button>}
            </div>
          ) : (
        ...
        <div className="pp-section">
          <div className="pp-section-hdr">
            ...
            <button className="pp-see-all" onClick={() => navigate(isClient ? '/client' : '/cockpit')}>Accéder au suivi â†’</button>
          </div>
          {displayProjects.length > 0 ? (
            ...
            <div style={{ textAlign: 'center', padding: '32px 24px', background: 'var(--s2)', borderRadius: 12 }}>
              ...
              {isOwner && <button className="pp-see-all" style={{ marginTop: 8 }} onClick={() => navigate('/cockpit')}>Créer un projet â†’</button>}
            </div>
          )}
        </div>
      ...
      <div className="pp-sticky">
        ...
        {isOwner ? (
          <button className="pp-sticky-btn" onClick={() => navigate('/cockpit')}>Mon Cockpit â†’</button>
        ) : (
          <>
            ...
            <button className="pp-sticky-btn" onClick={() => setShowContactModal(true)}>Contacter â†’</button>
          </>
        )}
      </div>

      {/* â•â•â• MODALS PROPRIÉTAIRE: Édition profil â•â•â• */}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 6 }}>
              {editSecteurs.map((s, i) => (
                <span key={i} style={{ padding: '4px 10px', borderRadius: 100, background: '#f3f4f5', fontSize: 12, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 5 }}>
                  {s}
                  <button onClick={() => setEditSecteurs(prev => prev.filter((_, j) => j !== i))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9a9a9a', fontSize: 14, lineHeight: 1 }}>À</button>
                </span>
              ))}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {editServices.map((s, i) => (
              <span key={i} style={{ padding: '5px 12px', borderRadius: 100, background: '#f3f4f5', fontSize: 12, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6 }}>
                {s}
                <button onClick={() => setEditServices(prev => prev.filter((_, j) => j !== i))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9a9a9a', fontSize: 14, lineHeight: 1 }}>À</button>
              </span>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            return (
              <div key={i} style={{ padding: '14px 16px', background: 'var(--s2)', borderRadius: 12, border: '1px solid var(--border-card)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  ...
                  <button onClick={() => setEditPortfolio(prev => prev.filter((_, j) => j !== i))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--err)', fontSize: 16, flexShrink: 0 }} title="Supprimer ce projet">À</button>
                </div>
                ...
                {imgs.length > 1 && <div style={{ fontSize: 10, color: 'var(--t4)', marginTop: 6 }}>{imgs.length} photos · La 1ère est la couverture affichée</div>}
                {imgs.length === 0 && <div style={{ fontSize: 10, color: 'var(--t4)', marginTop: 4 }}>Aucune photo — cliquez Â« Ajouter photos Â» pour en ajouter.</div>}
              </div>
            )
          })}
          {/* Add new project */}
          ...
          {editTeamList.map((m, i) => (
            <div key={i} style={{ padding: '10px 14px', background: '#f8f9fa', borderRadius: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                ...
                <button onClick={() => setEditTeamList(prev => prev.filter((_, j) => j !== i))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ba1a1a', fontSize: 14, flexShrink: 0 }}>À</button>
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
      ...
      {/* â•â•â• MODAL: Contacter le professionnel â•â•â• */}
      ...
      {/* â•â•â• MODAL: Inviter sur un projet â•â•â• */}
      ...
      {/* â•â•â• PORTFOLIO LIGHTBOX â•â•â• */}
      {portfolioViewer && createPortal(
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.93)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setPortfolioViewer(null)}>
          {/* Close */}
          <button onClick={() => setPortfolioViewer(null)} style={{ position: 'absolute', top: 20, right: 20, width: 42, height: 42, borderRadius: '50%', background: 'rgba(255,255,255,.1)', color: '#fff', border: 'none', fontSize: 22, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}>À</button>
          {/* Counter + caption */}
          ...
          {portfolioViewer.imgs.length > 1 && (
            <button onClick={e => { e.stopPropagation(); setPortfolioViewer(prev => ({ ...prev, idx: (prev.idx - 1 + prev.imgs.length) % prev.imgs.length })) }} style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', width: 46, height: 46, borderRadius: '50%', background: 'rgba(255,255,255,.1)', color: '#fff', border: 'none', fontSize: 26, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}>â€¹</button>
          )}
          {/* Main image */}
          ...
          {/* Next arrow */}
          {portfolioViewer.imgs.length > 1 && (
            <button onClick={e => { e.stopPropagation(); setPortfolioViewer(prev => ({ ...prev, idx: (prev.idx + 1) % prev.imgs.length })) }} style={{ position: 'absolute', right: 16, top: '50%', transform: 'translateY(-50%)', width: 46, height: 46, borderRadius: '50%', background: 'rgba(255,255,255,.1)', color: '#fff', border: 'none', fontSize: 26, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}>â€º</button>
          )}
          {/* Thumbnail strip */}
          {portfolioViewer.imgs.length > 1 && (
