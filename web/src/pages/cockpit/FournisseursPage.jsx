...
export default function FournisseursPage({ showToast, openModal, onNavigate }) {
  ...
  return (
    <div>
      <DSPageHeader title="Fournisseurs" subtitle="Catalogue · Évaluations · Vérification">
        ...
        <button className="btn btn-primary btn-sm" onClick={() => openModal('newFournisseur')}>+ Nouveau fournisseur</button>
      </DSPageHeader>
              ...
              <div key={idx} className="card" style={{ padding: 18 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 12, borderTop: '1px solid var(--border)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button className="btn btn-primary btn-sm" style={{ fontSize: 11, padding: '5px 10px' }} onClick={() => navigate('/profil')}>Profil</button>
                  </div>
                </div>
              </div>
            )
            ...
            <tbody>
              {filtered.sort((a, b) => b.note - a.note).map((f, i) => (
                <tr key={i}>
                  ...
                  <td><div style={{ display: 'flex', gap: 6 }}><button className="btn btn-sm" style={{ fontSize: 10, padding: '3px 8px' }} onClick={() => { setShowDevis(f); setDevis({ description: '', quantite: '', delai: '', projet: '', budget: '' }) }}>Devis</button><button className="btn btn-sm" style={{ fontSize: 10, padding: '3px 8px' }} onClick={() => navigate('/profil')}>Profil</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
