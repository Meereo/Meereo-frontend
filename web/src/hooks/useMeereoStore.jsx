...
export function MeereoProvider({ children }) {
    ...
    ;(async () => {
      try {
        ...
        const user = {
          ...
          company: me.company || '', phone: me.phone || '', avatar: me.avatar || '', wallet: 0,
        }
  ...
  const requestCloture = useCallback((data) => {
    ...
    updateStore(prev => {
      ...
      if (exists) {
        projects = projects.map(p => p.id === data.projectId ? { ...p, clotureStatus: projectStatus } : p)
      } else {
        // Copier le projet statique dans le store avec le nouveau statut
        projects = [...projects, { id: data.projectId, clotureStatus: projectStatus }]
      }
      return {
        ...prev,
  ...
  const respondCloture = useCallback((clotureId, accept, comment) => {
    updateStore(prev => {
      ...
      const projectStatus = accept ? 'CLOTURE_VALIDE_EXTERNE' : 'EN_COURS'
      return {
        ...prev,
